// This kata is designed to test your ability to extend the functionality of built-in ruby classes. In this case, we want you to extend the built-in Array class with the following methods: square(), cube(), average(), sum(), even() and odd().
//
// Explanation:
//
// square() must return a copy of the array, containing all values squared, the original array must not be changed
// cube() must return a copy of the array, containing all values cubed, the original array must not be changed
// average() must return the average of all array values, average() on an empty array must return NaN
// sum() must return the sum of all array values
// even() must return an array of all even numbers, the original array must not be changed
// odd() must return an array of all odd numbers, the original array must not be changed
// Examples:
//
// var numbers = [1, 2, 3, 4, 5];
// numbers.square(); // must return [1, 4, 9, 16, 25]
// numbers.cube(); // must return [1, 8, 27, 64, 125]
// numbers.average(); // must return 3
// numbers.sum(); // must return 15
// numbers.even(); // must return [2, 4]
// numbers.odd(); // must return [1, 3, 5]

// TODO
Array.prototype.square = function() {
  let workingArray = this.slice();
  console.log(workingArray);
  for(let i = 0; i < workingArray.length; i++){
    let number = workingArray[i];
    console.log(number, workingArray[i])
    number = number * number;
    console.log(number)
    workingArray[i] = number;
    console.log(workingArray)
  }
  return workingArray;
};

Array.prototype.cube = function() {
  let numbers = this.slice();
  console.log(numbers);
   for(let i = 0; i < numbers.length; i++){
      let number = numbers[i];
      console.log(number, numbers[i])
      let square = number * number;
      let cube = square * number;
      console.log(cube)
      numbers[i] = cube;
      console.log(numbers[i])
    }
    return numbers;
};

Array.prototype.average = function() {
  let workingNumbers = this.slice();
  let amount = workingNumbers.length;
  let sum = 0;
  for( let i = 0; i < workingNumbers.length; i++){
    sum = sum + workingNumbers[i];
  }
  let average = sum/amount;
  return average;
};

Array.prototype.sum = function() {
  let sum = 0;
  let workingArray = this.slice();
  for( let i = 0; i < workingArray.length; i++){
    sum += workingArray[i];
  }
  return sum;
};

Array.prototype.even = function() {
  let workingArray = this.slice();
  let even = [];
  for(let i = 0; i < workingArray.length; i++){
    if(workingArray[i] % 2 == 0){
      even.push(workingArray[i]);
    }
  }
  return even;
};

Array.prototype.odd = function() {
  let workingArray = this.slice();
  let odd = [];
  for(let i = 0; i < workingArray.length; i++){
    if(workingArray[i] % 2 != 0){
      odd.push(workingArray[i]);
    }
  }
  return odd;
};
